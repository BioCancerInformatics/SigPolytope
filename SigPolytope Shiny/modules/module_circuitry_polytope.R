...
# modules/mod_circuitry_polytope.R
# Módulo completo 3D: filtros → seleção → polytope
# + downloads: HTML (já existia) + dados (CSV/RDS) + PNG (opcional)
# ============================================================
  ...
  tagList(
    fluidRow(
      box(
        ...
        # -----------------------------
        # DOWNLOADS (pedido do chefe)
        # -----------------------------
        tags$div(
          style = "display:flex; flex-direction:column; gap:8px;",
          downloadButton(ns("dl_poly_html"), "Download 3D Polytope (.html)"),
          downloadButton(ns("dl_poly_data_csv"), "Download Polytope data (.csv)"),
          downloadButton(ns("dl_meta_row_csv"), "Download selected meta row (.csv)"),
          downloadButton(ns("dl_plot_png"), "Download plot snapshot (.png)")
        ),
        
        tags$p(
          style="margin-top:10px; font-size:12px; color:#666;",
          "Tip: HTML is the most reliable. PNG export depends on 'webshot2' + a headless browser."
        )
      ),
...
mod_circuitry_polytope_server <- function(id, tensor, embedding) {
  moduleServer(id, function(input, output, session) {
    ...
    # -------------------------------------------------------
    # Índice global direto do tensor
    # -------------------------------------------------------
    global_idx <- reactive({
      req(input$sel_circ)
      match(input$sel_circ, tensor$meta$Circuitries_id)
    ...
    # -------------------------------------------------------
    # Polytope: calcula só quando sel_circ muda
    # -------------------------------------------------------
    poly_obj <- eventReactive(input$sel_circ, {
      idx <- global_idx()
      req(!is.na(idx))
    ...
    # -------------------------------------------------------
    # Helpers: pacote "limpo" de exportação (dados usados no plot)
    # -------------------------------------------------------
    export_bundle <- reactive({
      req(input$sel_circ, poly_obj())
      idx <- global_idx()
      req(!is.na(idx))
      
      meta_row <- tensor$meta[idx, , drop = FALSE]
      
      list(
        circuitry_id = input$sel_circ,
        meta_row     = meta_row,
        vertices_all = poly_obj()$vertices_all,
        vertices_sig = poly_obj()$vertices_sig,
        vertices_int = poly_obj()$vertices_int,
        bary_sig_3d  = poly_obj()$bary_sig_3d,
        bary_int_3d  = poly_obj()$bary_int_3d,
    ...
    safe_id <- reactive({
      req(input$sel_circ)
      gsub("[^A-Za-z0-9]+", "_", input$sel_circ)
    })
    
    # -------------------------------------------------------
    # DOWNLOAD HTML (já existia)
    # -------------------------------------------------------
    output$dl_poly_html <- downloadHandler(
      filename = function() {
        paste0("polytope_", safe_id(), ".html")
      },
      content = function(file) {
        req(plot_obj())
    ...
    # -------------------------------------------------------
    # DOWNLOAD dados do polytope (CSV)
    # - exporta vertices + barycenters (tabulares)
    # -------------------------------------------------------
    output$dl_poly_data_csv <- downloadHandler(
      filename = function() paste0("polytope_data_", safe_id(), ".csv"),
      content = function(file) {
        b <- export_bundle()
        
        # CSV tabular: vertices_all + duas linhas de barycenters
        bary_df <- dplyr::bind_rows(
          tibble::tibble(side = "bary_sig", PC1 = b$bary_sig_3d["PC1"], PC2 = b$bary_sig_3d["PC2"], PC3 = b$bary_sig_3d["PC3"]),
          tibble::tibble(side = "bary_int", PC1 = b$bary_int_3d["PC1"], PC2 = b$bary_int_3d["PC2"], PC3 = b$bary_int_3d["PC3"])
        )
        
        out <- dplyr::bind_rows(
          b$vertices_all %>% dplyr::mutate(point_type = "vertex"),
          bary_df        %>% dplyr::mutate(point_type = "barycenter")
        )
    ...
    # -------------------------------------------------------
    # DOWNLOAD metadados da linha selecionada (CSV)
    # -------------------------------------------------------
    output$dl_meta_row_csv <- downloadHandler(
      filename = function() paste0("meta_row_", safe_id(), ".csv"),
      content = function(file) {
        b <- export_bundle()
        readr::write_csv(b$meta_row, file)
      }
    )
    
    # -------------------------------------------------------
    # DOWNLOAD PNG (opcional; depende de webshot2)
    # -------------------------------------------------------
    output$dl_plot_png <- downloadHandler(
      filename = function() paste0("polytope_", safe_id(), ".png"),
      content = function(file) {
        req(plot_obj())
        
        # Estratégia robusta:
        # 1) salva HTML temporário
        # 2) converte para PNG via webshot2 (se existir)
        if (!requireNamespace("webshot2", quietly = TRUE)) {
          stop("Package 'webshot2' not installed. Install it or use HTML download.")
        }
        
        tmp_html <- tempfile(fileext = ".html")
        htmlwidgets::saveWidget(plotly::as_widget(plot_obj()), tmp_html, selfcontained = TRUE)
