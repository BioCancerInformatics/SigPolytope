...
APP_HEAD_TAGS <- shiny::singleton(
  tags$head(
    ...
    tags$style(HTML("
      ...
      #app-loading-title {
        font-size: 28px;          /* ↑ ligeiramente maior */
        font-weight: 700;
        color: #111;
        margin-bottom: 12px;
      }
      ...
      #sigpolytope_splash .card {
        ...
        background: #ffffff;
        border-radius: 14px;
        padding: 30px 34px;      /* ↑ mais respiro */
        box-shadow: 0 10px 30px rgba(0,0,0,0.25);
      }

      #sigpolytope_splash h2 {
        margin-top: 0;
        font-size: 26px;         /* ↑ explícito */
        font-weight: 700;
        color: #111;
        margin-bottom: 14px;
      }
      ...
      #sigpolytope_splash .hint {
        margin-top: 18px;
        font-size: 14px;         /* ↑ de 13px */
        color: #666;
      }

      /* =========================
         Overview typography (ENTRY 3)
         ========================= */
      .overview-text p {
        font-size: 16px;
        line-height: 1.75;
  ...
  dashboardBody(
    ...
    tags$div(
      ...
      tags$div(
        class = "card",
        tags$h2("SigPolytope — Geometric Omic Signatures"),
        tags$p("Explore multi-omic regulatory circuitries as structured geometric objects rather than gene lists or vectors."),
        tags$div(class = "hint", "Click anywhere to start.")
      )
    ),
